...
const ProspectDetails = () => {
  ...
  const [prospectDetails, setProspectDetails] = useState([]); // Add a loading state
  const { prospectId } = useParams(); // Use useParams() to access dynamic params in the app directory
  console.log('pid==>' + prospectId);
  ...
  useEffect(() => {
    if (prospectId) {
      ...
      axios
        ...
        .then((response) => {
          console.log('data==>' + response.data.prospect);
          ...
          <div className="container mx-auto">
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="w-full lg:w-2/3">
                <div className="bg-white rounded-lg shadow-md overflow-hidden">
                  <ProspectTable dataProspect={prospectDetails} />
                </div>
              </div>
            </div>
          </div>
