...
const ProspectTable = ({ showOnlyAssigned = false }) => {
  ...
  const [filters, setFilters] = useState({
    nom: '', prenom: '', cin: '', telephone: '', email: '', statut: '',
  });
  ...
  useEffect(() => {
    ...
    fetchData_table_by_projet(
      { API_URL: 'prospects', dataKey: 'prospects', searchFields: ['nom', 'prenom', 'email', 'telephone', 'cin'] },
      filters, searchTerm, currentPage, rowsPerPage, accesstoken,
      setLoading, setError, setProspects, setTotalRows
    );
  }, [accesstoken, currentPage, rowsPerPage, searchTerm, filters, selectedProjet, user]);
    ...
    return filteredProspects.map((pro) => ({
      ...
      telephone:
        ...
        (pro.telephone && pro.telephone_num2 && pro.telephone_num2 !== 'null'
          ? ' / ' + pro.telephone_num2 : '') || 'Non spécifié',
      cin: pro.cin,
      client: pro.client,
      visites: pro.visites,
      appels: pro.appels,
      origin: pro.origin,
      date: pro.created_at ? format(new Date(pro.created_at), 'yyyy-MM-dd') : '',
      ...
      date_affectation: pro.date_affectation ? format(new Date(pro.date_affectation), 'yyyy-MM-dd HH:mm') : '',
      date_traitement: pro.date_traitement ? format(new Date(pro.date_traitement), 'yyyy-MM-dd HH:mm') : '',
      prospect: pro,
    }));
  };
  ...
  const getFilteredTotalRows = () => {
    if (showOnlyAssigned && isCommercialUser && user?.id) {
      const filteredCount = prospects.filter(pro =>
  ...
  const columns = [
        ...
        <div>
          <span>
            {row.commercial_affecte
              ? `${row.commercial_affecte.name || ''} ${row.commercial_affecte.prenom || ''}`
              : ''}
          </span>
          {row.affecte_par_admin && (
            <div className="text-xs text-gray-500">
              Affecté par: {row.affecte_par_admin.name} {row.affecte_par_admin.prenom}
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <div>
            <span className={`px-2 py-1 rounded text-sm font-semibold ${getProspectStatusColor(row.statut)}`}>
              {row.statut}
            </span>
            {row.traite_par_user && (
              <div className="text-xs text-gray-500 mt-1">
                Traité par: {row.traite_par_user.name} {row.traite_par_user.prenom}
    ...
    { key: 'origin', label: 'Origine', render: (row) => <span>{row.origin}</span> },
      ...
      render: (row) => (
        <div className="flex gap-3 items-center">
          <div title="Voir détails">
            <Eye className="w-4 h-4 !text-blue-500 hover:text-blue-700 cursor-pointer" onClick={() => handleShow(row.id)} />
          </div>
          <div title="Modifier">
            <Pencil className="w-4 h-4 !text-yellow-500 hover:text-yellow-700 cursor-pointer" onClick={() => handleEdit(row.id)} />
          </div>
          <div title="Traiter">
            <Check className="w-4 h-4  hover:text-['rgb(87,80,129)']-700 text-['rgb(87,80,129)'] cursor-pointer" onClick={() => handleraiter(row.id, row.telephone, row.nomComplet)} />
          </div>
          <div title="Convertir en visite">
            <RefreshCw className="w-4 h-4 !text-green-500  cursor-pointer" onClick={() => handle_convert_to_visite(row.prospect)} />
          </div>
          {row.client == null && row.visites.length == 0 && row.appels == null && (
            <div title="Supprimer utilisateur">
              <Trash2 className="w-4 h-4 !text-red-500 hover:text-red-700 cursor-pointer" onClick={() => { setSelectedId(row.id); setShowDeleteModal(true); }} />
            </div>
          )}
        </div>
      ),
  ...
  const handleShow = (prospectId) => router.push(`/crm/prospects/${prospectId}`);
  const handleEdit = (ProspectId) => router.push(`${ENDPOINTS.PROSPECTS}?id=${ProspectId}&action=edit`);
  const handleraiter = (Id, num_tel, nom_prenom) => { setOpen_traite(!open_traite); setId_traite(Id); setTel_num(num_tel); setNomPrenom(nom_prenom); };
  const handle_convert_to_visite = (row) => { localStorage.setItem('selectedProspect', JSON.stringify({ dataProspect: row })); router.push(`${ENDPOINTS.VISITES}?action=add`); };
  const handleFilterChange = (field, value) => setTempFilters((prev) => ({ ...prev, [field]: value }));
    ...
    if (showAffecterModal && selectedProjet?.id) {
      const fetchCommercials = async () => {
        try {
          ...
          const commercialUsers = data.users ? data.users.map((item) => item.user).filter(Boolean) : [];
  ...
  const filteredCommercials = commercials.filter((user) =>
    `${user.name} ${user.prenom}`.toLowerCase().includes(searchCommercial.toLowerCase())
  );
    ...
    <>
      <div className="reflative bg-white rounded-lg shadow-md p-4">
        <Table
          ...
          extraActions={
            ...
            !isCommercialUser && checkedProspects.length > 0 && (
              <button
          ...
          filterComponent={
            <div className="space-y-4 ">
              <div className="grid gap-5" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
                <Input type="text" label="Cin" value={tempFilters.cin} onChange={(e) => handleFilterChange('cin', e.target.value)} className="h-10 px-3 py-2 rounded-md border border-gray-300 w-full text-sm" />
                <Input type="text" label="Nom" value={tempFilters.nom} onChange={(e) => handleFilterChange('nom', e.target.value)} className="h-10 px-3 py-2 rounded-md border border-gray-300 w-full text-sm" />
                <Input type="text" label="Prénom" value={tempFilters.prenom} onChange={(e) => handleFilterChange('prenom', e.target.value)} className="h-10 px-3 py-2 rounded-md border border-gray-300 w-full text-sm" />
                <Input type="number" label="Téléphone" value={tempFilters.telephone} onChange={(e) => handleFilterChange('telephone', e.target.value)} className="h-10 px-3 py-2 rounded-md border border-gray-300 w-full text-sm" />
                <Input type="email" label="Email" value={tempFilters.email} onChange={(e) => handleFilterChange('email', e.target.value)} className="h-10 px-3 py-2 rounded-md border border-gray-300 w-full text-sm" />
                <SelectInput value={tempFilters.statut} onChange={(value) => handleFilterChange('statut', value)} options={Object.values(Statuts_Prospect).map((data) => ({ value: data.id, label: data.label }))} label="Choisir un Statut" className="h-10 text-sm w-full" />
              </div>
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={applyFilters} className="px-3 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700">Appliquer les filtres</button>
                <button type="button" onClick={resetFilters} className="px-3 py-2 bg-gray-400 text-white text-sm rounded hover:bg-gray-500">Réinitialiser</button>
              </div>
            </div>
          }
          rowClassName={(row) => {
        ...
        {!isCommercialUser && showAffecterModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
              <button className="absolute top-2 right-2 text-gray-400 hover:text-gray-600 text-2xl" onClick={handleCloseAffecter} aria-label="Fermer">&times;</button>
              <h2 className="text-xl font-semibold mb-4">Affecter des prospects</h2>
              ...
              <div className="mb-4 flex gap-2">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md border ${assignMode==='selective' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white border-gray-300 text-gray-700'}`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md border ${assignMode==='auto' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white border-gray-300 text-gray-700'}`}
                >
              ...
              {assignMode === 'selective' && (
                <div className="mb-4">
                  <label className="block mb-2 font-medium text-gray-700">Sélectionner un commercial</label>
                  <div className="relative">
                    ...
                    >
                      <span>
                        {selectedCommercial
                          ? (() => {
                              const user = commercials.find((u) => u.id == selectedCommercial);
                              return user ? `${user.name} ${user.prenom}` : "Sélectionner un commercial";
                            })()
                          : "Sélectionner un commercial"}
                      </span>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={showCommercialDropdown ? "M5 15l7-7 7 7" : "M19 9l-7 7-7-7"} />
                      </svg>
                    </button>
                      ...
                      <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
                        ...
                        <div className="py-1">
                          {filteredCommercials.length === 0 ? (
                            <div className="px-4 py-2 text-sm !text-gray-500">Aucun commercial trouvé</div>
                          ) : (
      ...
      {open_traite && (
        <Modal isVisible={true} onClose={() => setOpen_traite(false)}>
          <Modal_Traite nom_prenom={nom_prenom} num_tel={num_tel} id={traite_id} onClose={() => setOpen_traite(false)} />
        </Modal>
      )}
      {showImportModal && (
