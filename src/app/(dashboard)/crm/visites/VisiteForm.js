...
const VisiteForm = ({ prospect_id, origin, client_reservations = [] }) => {
  ...
  const [display_nom, setdisplay_nom] = useState(false);
    ...
    const newExpanded = expanded
      ...
      .map((panel) => {
        ...
        if (match) {
          ...
          if (panelIndex > indexToDelete + 1) {
            return `panel_bien${panelIndex - 1}`;
          }
        }
        return panel;
      });
                        ...
                        {watch("loading_b_pre") == false && (
                          <>
                            ...
                            {/*Number(watch('interet')) == 1 && (
                              <>
                                <TextField
                                  label="Nombre de Biens à ajouter first:"
                                  name="nb_bien_added"
                                  type="number"
                                  control={control}
                                  errors={{
                                    ...errors,
                                    nb_bien_added: formSubmitted && Number(watch('interet')) == 1 && !watch('nb_bien_added') && watch('nb_bien_added') !== 0
