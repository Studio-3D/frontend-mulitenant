...
export default function CommissionForm({ id = null, onComplete }) {
  ...
  const handleChange = (event, index) => {
    ...
    if (name === 'pourcentage') {
      if (value === '0' || parseFloat(value) <= 0) {
        setMsg_alert(`Le pourcentage ne doit pas être égal à zéro ni être négatif à la ligne ${index + 1}`);
      } else if (parseFloat(value) > 100) {
        setMsg_alert(`Le pourcentage ne doit pas dépasser 100% à la ligne ${index + 1}`);
      } else {
        setMsg_alert(null);
      }
    }
    ...
    if (name === 'de') {
      ...
      if (index > 0) {
        ...
        if (parseFloat(value) <= parseFloat(inputs[index - 1].a)) {
          setMsg_alert(`La valeur "De" ne doit pas débuter par ${value} à la ligne ${index + 1}`);
        } else {
          setMsg_alert(null);
        }
      }
  ...
  const validateForm = () => {
    ...
    for (let i = 1; i < inputs.length; i++) {
      if (parseFloat(inputs[i].de) <= parseFloat(inputs[i-1].a)) {
        setMsg_alert(`La valeur "De" ne doit pas débuter par ${inputs[i].de} à la ligne ${i + 1}`);
        return false;
      }
    }
    ...
    for (let i = 0; i < inputs.length; i++) {
      if (!inputs[i].de || !inputs[i].a || !inputs[i].pourcentage) {
        setMsg_alert(`Tous les champs doivent être remplis à la ligne ${i + 1}`);
        return false;
      }
      
      if (parseFloat(inputs[i].pourcentage) <= 0 || parseFloat(inputs[i].pourcentage) > 100) {
        setMsg_alert(`Le pourcentage doit être entre 1 et 100 à la ligne ${i + 1}`);
        return false;
      }
    }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm">
      <h2 className="text-xl font-semibold mb-6">Configuration des commissions</h2>
      ...
      <form onSubmit={handleSubmit}>
          ...
          {inputs.map((input, index) => (
            <div key={index} className="grid grid-cols-1 md:grid-cols-7 gap-4 mb-4 items-end">
              <div className="md:col-span-1 flex items-center justify-end h-[38px]">
                <span className="text-sm font-medium text-gray-700">Configuration {index + 1}</span>
              </div>
              <div className="md:col-span-1">
        ...
        <div className="flex justify-end gap-3 mt-8 border-t pt-4">
          <button
            type="submit"
            disabled={submitting || msg_alert !== null || commissionMontant === ''}
