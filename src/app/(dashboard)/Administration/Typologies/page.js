...
export default function TypologiesPage() {
  ...
  if (!selectedProjet && !action) {
    return <div>Veuillez sélectionner un projet pour accéder aux typologies.</div>;
  }
  ...
  if (action === 'add' || action === 'edit') {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6">
          <Link href="/Administration/Typologies" className="inline-flex items-center gap-2 text-[#009FFF] hover:text-blue-800">
            <TbArrowBackUp className="text-xl" />
            <span>Retour à la liste</span>
          </Link>
        </div>
        <h1 className="text-2xl font-bold mb-6">
          {action === 'add' ? 'Ajouter une nouvelle typologie' : 'Modifier la typologie'}
        </h1>
        <TypologieForm
