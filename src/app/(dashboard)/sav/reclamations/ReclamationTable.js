...
const ReclamationTable = (prestId) => {
  ...
  const fetchPrestataires = async (service_id) => {
    try {
      ...
      const filtered = response.data.prestataire.filter(
  ...
  const handleSubmitReclamation = (e) => {
    ...
    if (dialogType === 'traiter') {
      if (!formValues.prestataire_id || !formValues.date_intervention) {
        toast.error(
          'Veuillez remplir tous les champs obligatoires (Prestataire et Date).'
        );
        return;
      }
      ...
      if (!formValues.statut || !formValues.date_fin_inter) {
        toast.error(
          'Veuillez remplir tous les champs obligatoires (Statut et Date de Fin).'
        );
        return;
      }
    }
      ...
      .then(() => {
        toast.success(
          dialogType === 'traiter'
            ? 'Réclamation traitée !'
            : `Réclamation ${
                formValues.statut == 3 ? 'résolue' : 'non résolue'
              } !`
        );
  ...
  const formatData = () => {
    return reclamations?.map((rec) => ({
      ...
      service: rec?.service?.nom ? rec?.service?.nom : '',
  ...
  const isPrestIdEmpty =
    !prestId ||
    (typeof prestId === 'object' && Object.keys(prestId).length === 0);
          ...
          <div className="space-y-4 rounded-lg">
            ...
            >
              <Input
                ...
                onChange={(e) =>
                  handleFilterChange('date_reclamation', e.target.value)
                }
              ...
              <Input
                ...
                onChange={(e) =>
                  handleFilterChange('date_intervention', e.target.value)
                }
                className="h-10 px-3 py-2 rounded-md border border-gray-300 w-full text-sm"
              />
              ...
              <Input
                ...
                onChange={(e) =>
                  handleFilterChange('date_fin_intervention', e.target.value)
                }
                className="h-10 px-3 py-2 rounded-md border border-gray-300 w-full text-sm"
              />
              ...
              <Input
                ...
                onChange={(e) =>
                  handleFilterChange('propriete_dite_bien', e.target.value)
                }
                className="h-10 px-3 py-2 rounded-md border border-gray-300 w-full text-sm"
              />
              ...
              <Input
                ...
                onChange={(e) =>
                  handleFilterChange('prestataire', e.target.value)
                }
                className="h-10 px-3 py-2 rounded-md border border-gray-300 w-full text-sm"
              />
