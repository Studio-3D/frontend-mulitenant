...
import { Eye, Check, Wrench } from 'lucide-react';
...
const ReclamationTable = ({}) => {
  ...
  // Update filters when selectedProjet changes
  useEffect(() => {
    setFilters((prev) => ({ ...prev, projet_id: selectedProjet?.id }));
    setTempFilters((prev) => ({ ...prev, projet_id: selectedProjet?.id }));
  }, [selectedProjet]);
  ...
  const resetFilters = () => {
    const reset = {
      ...
      code_reservation: '',
      etat: '',
      projet_id: selectedProjet?.id, // Keep the current projet_id
    };
    setFilters(reset);
  ...
  const handleSubmitReclamation = (e) => {
    ...
    axios
      .post(
        `${APIURL.ROOTV1}/traiter_reclamation_client/${selectedId}`,
        formData,
        {
          headers: {
        ...
        return (
          <span
            className={`px-2 py-1 rounded text-sm font-semibold ${
              etatStyles[row.etat] || 'bg-gray-200 !text-gray-800'
            }`}
          >
