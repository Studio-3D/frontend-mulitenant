...
const HistoImpoForm = ({ id = null }) => {
  ...
  // Load histoImportation data if editing
  useEffect(() => {
    if (id) {
      fetchhistoImportationData(id);
    }
  }, [id]);

  const fetchhistoImportationData = async (histoImportationId) => {
    setLoading(true);
    try {
      const response = await axios.get(
        `${APIURL.histoImportations}/${histoImportationId}`,
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        }
      );

      if (response.data?.histoImportation) {
        const histoImportationData = response.data.histoImportation;
        // Remplir les champs avec les données de histoImportation
        setValue("nom", histoImportationData.nom || "");
        setValue("cin", histoImportationData.cin || "");
        setValue("prenom", histoImportationData.prenom || "");
        setValue("telephone", histoImportationData.telephone || "");
        setValue("email", histoImportationData.email || "");
        setValue("adresse", histoImportationData.adresse || "");
        setValue("service_id", histoImportationData.service?.id || "");
        setValue("civilite", getSCodeCivilite(histoImportationData.civilite) || "");
      }
    } catch (error) {
      console.error("Error fetching histoImportation data:", error);
  ...
  const fetchServices = async () => {
    try {
      ...
      const response = await axios.get(
        `${APIURL.ROOT}/v1/projets/${selectedProjet?.id}/ServiceshistoImportations/`,
        {
          headers: {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      let url = APIURL.histoImportations;
      let method = "post";
      ...
      toast.success(
        id
          ? "histoImportation modifié avec succès"
          : "histoImportation ajouté avec succès"
      );
  ...
  // Ajoute ça dans histoImportationForm.jsx
...
const fetch_event_by_param = async (route, value, param) => {
  try {
    ...
    if (res.data.histoImportation != null) {
      setDisabled(true);
      setInfo_client(
        `Le ${param} que vous avez saisi appartient au histoImportation : ${res.data.histoImportation.nom} ${res.data.histoImportation.prenom}`
      );
    }
  } catch (error) {
    console.error(error);
...
const fetch_cin_unique = async (value) => {
  try {
    const res = await axios.get(
      `${APIURL.ROOTV1}/get_info_cin_histoImportation_unique/${id}/${value}`, 
      {
        headers: { Authorization: `Bearer ${accessToken}` },
      }
    );
    ...
    if (res.data.pres_count > 0) {
      setInfo_client('Le CIN que vous avez saisi appartient à un autre histoImportation.');
      setDisabled(true);
    }
  } catch (error) {
    console.error(error);
  ...
  if (name === 'cin') {
    if (value.length >= 3) {
      ...
      window.cinTimeout = setTimeout(async () => {
        await fetch_event_by_param('search_histoImportation_by_param', value, 'cin');
    ...
    if (value.length >= 10) {
      ...
      window.telTimeout = setTimeout(() => {
        fetch_event_by_param('search_histoImportation_by_param', value, 'telephone');
      }, 1000);
    }
  }
};
...
<div className="p-3">
  ...
  <div className="flex items-center justify-start">
    <BreadCrumb
      ...
      step={`${id ? "Modifier" : "Ajouter"} un histoImportation`}
    />
  </div>
