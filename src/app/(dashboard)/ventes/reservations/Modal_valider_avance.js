...
export default function Modal_valider_avance({ onClose, id, first_num_recu }) {
  const { token } = useAuth();
  const accessToken = token || localStorage.getItem('accessToken');
  const [loading, setLoading] = useState({ form: false });
  const [backendErrors, setBackendErrors] = useState(null);
  const defaultValues = {
    statut_res: '', // Make sure all fields you need are presen
    with_avance: '',
    statut_av: '',
    n_remise:'',
    date_encaiss:'',
  ...
  const validationSchemaRef = useRef(
    yup.object().shape({
      action: yup.string().required('Action est requis'),
    })
  );
  const {
    control,
