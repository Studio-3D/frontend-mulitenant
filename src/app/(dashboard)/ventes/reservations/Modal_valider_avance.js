    ...
    axios({
      ...
      .then((res) => {
        ...
        if (res.status == 200) {
          ...
          if (res_show && onSuccess) {
            // If we're in res_show mode, call the success callback
            console.log(
              '🔍 Calling onSuccess callback from handle_annuler_avance'
            );
            onSuccess();
          } else {
