...
const formatCurrency = (amount) => {
  return amount ? new Intl.NumberFormat('fr-FR').format(amount) + ' DH' : '0 DH';
};
...
const ClientFullPDF = ({ client, reservations, visites, user }) => {
  ...
  const formatStatutReservation = (statut) => {
    switch(statut) {
      case '1': return { text: 'Validé', style: styles.statusValid };
      case '2': return { text: 'Refusé', style: styles.statusRejected };
      case '3': return { text: 'En Attente', style: styles.statusPending };
      default: return { text: 'Inconnu', style: {} };
    }
  };

  const formatInteret = (interet) => {
    switch(interet) {
      case '1': return { text: 'Intéressé', style: styles.interesse };
      case '2': return { text: 'Réceptif', style: styles.receptif };
      case '3': return { text: 'Perdu', style: styles.perdu };
      case '4': return { text: 'Injoignable', style: styles.injoignable };
      default: return { text: 'Non spécifié', style: {} };
    }
  };

  const formatStatutVisite = (statut) => {
    switch(statut) {
      case '1': return { text: 'Pré-Réservation', style: styles.preReservation };
      case '2': return { text: 'Vendu', style: styles.vendu };
      case '3': return { text: 'Pré-Réservation Perdu', style: styles.preReservationPerdu };
      case '4': return { text: 'Réservation Perdu', style: styles.reservationPerdu };
      case '5': return { text: 'Pré-Réservation Vendu', style: styles.preReservationVendu };
      default: return { text: 'Non spécifié', style: {} };
    }
  };
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            <Text style={styles.companyName}>{user?.societe?.raison_sociale}</Text>
              ...
              {reservations.map((row, index) => {
                ...
                return (
                  <View style={styles.tableRow} key={index}>
                    ...
                    <Text style={styles.tableCell}>{formatDate(row.date_reservation)}</Text>
                    <Text style={styles.tableCell}>{row.bien?.propriete_dite_bien || ''}</Text>
                    <Text style={styles.tableCell}>{row.bien?.projet?.nom || ''}</Text>
                    <Text style={styles.tableCell}>{formatCurrency(row.prix)}</Text>
                    <Text style={styles.tableCell}>{formatCurrency(row.avances_sum_montant)}</Text>
                    <Text style={styles.tableCell}>
                      {formatCurrency(row.prix - (row.avances_sum_montant || 0))}
                    </Text>
                    <Text style={styles.tableCell}>{formatFinancement(row.mode_financement)}</Text>
                    <Text style={[styles.tableCell, statut.style]}>{statut.text}</Text>
                  </View>
                );
              })}
            </View>
              ...
              {visites.map((row, index) => {
                ...
                return (
                  <View style={styles.tableRow} key={index}>
                    ...
                    <Text style={styles.tableCell}>{row.nom_cc} {row.prenom_cc}</Text>
                    <Text style={[styles.tableCell, interet.style]}>{interet.text}</Text>
                    <Text style={[styles.tableCell, statutVisite.style]}>{statutVisite.text}</Text>
                    <Text style={styles.tableCell}>{row.propriete_dite_bien || ''}</Text>
                  </View>
                );
              })}
            </View>
        ...
        <View style={styles.footer}>
          <Text>Document généré le {formatDate(new Date())} - © {user?.societe?.raison_sociale} {new Date().getFullYear()}</Text>
