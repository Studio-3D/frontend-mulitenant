...
const AutocompleteMultipleDes = ({
  ...
  useEffect(() => {
    ...
    const newSelected = value
      .map((val) => {
        ...
        return options.find(
          (opt) => String(opt[actualValueKey]) == String(val)
        );
      })
      .filter(Boolean);
  ...
  const filteredOptions =
    inputValue.trim() === ''
      ...
      : availableOptions.filter((opt) => {
          const fullName = `${opt.client?.nom || opt.nom || ''} ${
            opt.client?.prenom || opt.prenom || ''
          }`.toLowerCase();
  ...
  return (
    <div className="w-full relative">
          ...
          {selectedOptions.map((option) => (
            <div
              key={
                ...
                `selected-${Math.random().toString(36).substr(2, 9)}`
              }
      ...
      {backendErrors[name] && (
        <p className="mt-1 text-sm text-red-600">{backendErrors[name][0]}</p>
      )}
    </div>
  );
};
