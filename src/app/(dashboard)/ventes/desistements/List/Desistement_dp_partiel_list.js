...
export default function Desistement_dp_partiel_list() {
  ...
  const formatData = () => {
    return data.map((desistement) => {
      ...
      // Process desisteurs partiel
      if (desistement.aquereurs_partiel?.length > 0) {
        desistement.aquereurs_partiel.forEach((a) => {
          if (a?.aquereur?.client) {
            desisteursPartiel.push({
      ...
      if (desistement.nouvel_aquereurs_desistements?.length > 0) {
        desistement.nouvel_aquereurs_desistements.forEach((a) => {
          nouveauxAquereurs.push({
  ...
  const columns = [
    ...
    {
      key: 'desisteursPartiel',
      label: 'Désisteurs',
      render: (row) => (
        <div className="flex flex-col gap-1">
          {row.desisteursPartiel.map((client, index) =>
            client.id ? (
              <Link
                key={index}
                href={`/ventes/clients/${client.id}`}
  ...
  return (
    <div className="relative bg-white shadow-md rounded-lg px-4 py-4">
      ...
      <Table
        ...
        name_file_export={'desistements_partiel_export'}
        columns={columns}
        data={formatData()}
        totalRows={totalRows}
        loading={loading}
