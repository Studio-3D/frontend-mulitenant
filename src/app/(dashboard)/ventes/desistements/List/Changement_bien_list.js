...
export default function Changement_bien_list() {
  ...
  const formatData = () => {
    return data.map((desistement) => {
      ...
      // Fall back to aquereurs_ancien if no aquereurs
      ...
      return {
        ...
        ancien_bien: desistement.bien_ancien?.propriete_dite_bien || '',
        nouveau_bien: desistement.bien_nouveau?.propriete_dite_bien || '',
        montant_ajouter: desistement.montant_a_ajouter
          ? `${desistement.montant_a_ajouter.toLocaleString()} DH`
          : '',
  ...
  const data_to_export = () => {
    return data.map((desistement) => {
      ...
      return {
        ...
        'Ancien Bien': desistement.bien_ancien?.propriete_dite_bien || '',
        'Nouveau Bien': desistement.bien_nouveau?.propriete_dite_bien || '',
        'Montant à Ajouter': desistement.montant_a_ajouter
          ? `${desistement.montant_a_ajouter.toLocaleString()} DH`
          : '',
  ...
  return (
    <div className="reflative bg-white shadow-md rounded-lg p-4">
      <p className="text-lg font-semibold mb-4">
        Désistements Changement de Bien{' '}
        {etat_desistement == 5
          ? 'En Attentes'
          : etat_desistement == 0
          ? 'Encours'
      ...
      <Table
        ...
        name_file_export={'desistements_change_bien_export'}
        columns={columns}
        data={formatData()}
        totalRows={totalRows}
        loading={loading}
