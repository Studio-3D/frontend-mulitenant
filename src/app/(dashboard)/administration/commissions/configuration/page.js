...
export default function CommissionConfigForm({ onClose, onSuccess }) {
  ...
  const selectedProjet_id = JSON.parse(localStorage.getItem('selectedProjet'))?.id
  ...
  const fetchData_commission_montant = async () => {
    setLoading(true)
    try {
      const response = await axios.get(`${APIURL.ROOTV1}/commission_montant/` + selectedProjet_id, {
        headers: {
  ...
  const fetchData_Configuration = async () => {
    try {
      const res = await axios.get(`${APIURL.ROOTV1}/configurations_commissions/${selectedProjet_id}`, {
  ...
  const handleChange = (event, index) => {
    ...
    if (name === 'pourcentage') {
      if (value == 0 || value < 0) {
        setMsg_alert('Le pourcentage ne doit pas être égal à zéro ni être négatif. Ligne ' + (index + 1))
      } else if (value > 100) {
        setMsg_alert('Le pourcentage ne doit pas dépasser 100 % dans la ligne ' + (index + 1))
      } else {
        setMsg_alert(null)
      }
    }
    ...
    if (name === 'de' && index > 0) {
      if (value <= +inputs[index - 1]['a']) {
        setMsg_alert('La case De ne doit pas débuter par ' + value + ' à la ligne : ' + (index + 1))
      } else {
        setMsg_alert(null)
      }
    }
  ...
  const onSubmit = data => {
    inputs.forEach((ar, index) => {
      if (index > 0 && ar.de <= +inputs[index - 1]['a']) {
        setMsg_alert('La case De ne doit pas débuter par ' + ar.de + ' à la ligne : ' + (index + 1))
      }
    })
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h2 className="text-2xl font-semibold mb-6 text-center">Ajouter ou Modifier une Commission</h2>

      {msg_alert && (
        <div className="bg-yellow-100 border border-yellow-400 text-black-700 px-4 py-3 rounded mb-4 text-center" role="alert">
          {msg_alert}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4">
          <label htmlFor="commission_montant" className="w-full sm:w-1/4 text-gray-700 font-medium">
            Montant Fixe
          </label>
          <input
        ...
        <div className="bg-indigo-700 rounded p-2 text-white text-center font-semibold mb-4">Configuration</div>
        ...
        {inputs.map((item, index) => (
          ...
          >
            <div className="w-full sm:w-1/6 mb-2 sm:mb-0 font-semibold text-indigo-600">Configuration : {index + 1}</div>
        ...
        <div className="flex justify-end space-x-4">
          ...
          <Button
            type="submit"
            disabled={loading.form || watch('commission_montant') === '' || msg_alert != null}
            className={`px-6 py-2 rounded text-white ${
              loading.form || watch('commission_montant') === '' || msg_alert != null
