...
const Trait_accCommissionTable = ({type}) => {
  ...
  render: (row) => (
    ...
    >
      <strong>{`${row.user?.name || ''} ${row.user?.prenom || ''}`.trim()}</strong>
    </Link>
  ),
},
...
const formatData = () => {
  return commissions.map((com) => {
    const rawDate = type === 1 ? com?.date_traitement : type === 0 ? com?.created_at : null;
    let formattedDate = '';
    if (rawDate) {
...
const data_to_export = () => {
  return commissions.map((cm) => {
    const dateValue = type === 1 ? cm?.date_traitement : type === 0 ? cm?.created_at : null;
    let formattedDate = '';
    if (dateValue) {
