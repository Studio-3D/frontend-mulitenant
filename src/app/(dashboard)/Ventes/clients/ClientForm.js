      ...
      const fetchClientData = async () => {
        try {
          ...
          if (response.data && response.data.client) {
            ...
            age: client.age !== undefined && client.age !== null ? client.age : '',  // This line ensures 0 is not replaced by ''
 ...
 const handleSelectChange = (field, value) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.telephone_num1) {
      ...
      errors.telephone_num1 = "Le téléphone 1 doit contenir entre 10 et 14 chiffres.";
    }

    // Validation pour telephone_num2 (facultatif)
    if (formData.telephone_num2 && !/^\d{10,14}$/.test(formData.telephone_num2)) {
      errors.telephone_num2 = "Le téléphone 2 doit contenir entre 10 et 14 chiffres.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Le client a été ${isEditing ? "modifié" : "créé"} avec succès`);
  ...
  <Grid style={{ marginTop: '10px', marginBottom: '10px' }}>
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {formData.type_client == 2 && (
            <InputSelect
              ...
              onChange={(option) => {
                const partenaireSelected = partenaires.find(p => p.id === option?.value);
                handleSelectChange("partenaire_id", option?.value || null);
              }}
              error={validationErrors.partenaire_id || backendErrors.partenaire_id}
              isLoading={loading}
              required
            />
          )}
          ...
          <Input label="Nom" type="text" name="nom" value={formData.nom} onChange={handleChange}
            error={validationErrors.nom || backendErrors.nom}  required/>

          <Input label="Prénom" type="text" name="prenom" value={formData.prenom} onChange={handleChange}
            error={validationErrors.prenom || backendErrors.prenom} required />
          ...
          <Input
            ...
            required
            error={validationErrors.telephone_num1 || backendErrors.telephone_num1} />
          ...
          <Input label="Ville" type="text" name="ville" value={formData.ville} onChange={handleChange}
            error={validationErrors.ville || backendErrors.ville} />

          <Input label="Pays" type="text" name="pays" value={formData.pays} onChange={handleChange}
            error={validationErrors.pays || backendErrors.pays} />

          <Input label="Profession" type="text" name="profession" value={formData.profession}
            onChange={handleChange} error={validationErrors.profession || backendErrors.profession} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <Input label="Lieu de naissance" type="text" name="lieu_naissance" value={formData.lieu_naissance}
            onChange={handleChange} error={validationErrors.lieu_naissance || backendErrors.lieu_naissance} />

          <Input label="Nationalité" type="text" name="nationalite" value={formData.nationalite}
            onChange={handleChange} error={validationErrors.nationalite || backendErrors.nationalite} />

          <Input label="Date de naissance" type="date" name="date_naissance" value={formData.date_naissance}
            onChange={(e) => {
              handleChange(e);
              calculate_age(e.target.value);
            }} 
            error={validationErrors.date_naissance || backendErrors.date_naissance} />

          <Input label="Âge" type="number" name="age" value={formData.age} disabled />

            {(formData.age !== '' && (formData.age < 18 || formData.age === 0)) && (
                        <>
            <Input label="Nom Responsable" type="text" name="nom_responsable" value={formData.nom_responsable}
              onChange={handleChange} error={validationErrors.nom_responsable || backendErrors.nom_responsable} required/>

            <Input label="Relation Familiale" type="text" name="relation_familliale" value={formData.relation_familliale}
              onChange={handleChange} error={validationErrors.relation_familliale || backendErrors.relation_familliale} required/>
          </>
          )}
        ...
        <h4 className="text-xl font-bold text-blue-700 mb-4 border-b-2 border-blue-300 pb-2 flex items-center gap-2">
          <svg className="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M5.121 17.804A6.994 6.994 0 0012 21a6.994 6.994 0 006.879-3.196M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
          </svg>
          Situation familiale
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <SelectInput
            ...
            onChange={(value) =>
              handleChange({ target: { name: 'situation_familliale', value } })
            }
          ...
          {formData.situation_familliale == 2 && (
            <>
              ...
              <Input
                ...
                onChange={handleChange}
                error={validationErrors.date_mariage || backendErrors.date_mariage}
                required
              />
              <Input
                ...
                onChange={handleChange}
                error={validationErrors.lieu_mariage || backendErrors.lieu_mariage}
                required
              />
            </>
          )}
