...
const CoefficientDialog = ({ isOpen, onClose, onSave, tranche, action }) => {
  const [formData, setFormData] = useState({
    coefficient: tranche?.coefficient_tranche?.coefficient || '',
  });
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          ...
          <div className={`p-6 ${action === 0 ? 'bg-purple-600' : 'bg-orange-500'} text-white`}>
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-semibold">
                {action === 0 ? 'Ajouter Coefficient' : 'Modifier Coefficient'}
              </h3>
              <button 
                onClick={onClose}
          ...
          {action === 1 && (
            <div className="px-6 py-3 bg-red-200 !text-red-700 flex items-center gap-2">
              ...
              <p className="text-sm">
                Si vous modifiez le coefficient, tous les TVA des biens correspondant à cette tranche seront recalculés.
              </p>
            </div>
          )}
...
export default CoefficientDialog;
