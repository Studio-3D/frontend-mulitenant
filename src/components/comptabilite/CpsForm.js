...
const CpsForm = ({ cps, onSave, onCancel }) => {
  ...
  const schema = yup.object().shape({
    ...
    cout: yup.number().required('Le coût est requis').typeError('Le coût doit être un nombre'),
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white rounded-lg">
      <h2 className="text-2xl font-bold mb-6">{cps ? 'Modifier' : 'Ajouter'} un CPS</h2>
      ...
      <form onSubmit={handleSubmit(onSubmitForm)}>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`px-4 py-2 rounded-md shadow-sm text-sm font-medium text-white ${loading ? 'bg-blue-300' : 'bg-blue-600 hover:bg-blue-700'}`}
          >
