  ...
  tranche: {
    ...
    columns: (user, handleDelete) => [
      { key: 'nom', label: 'Tranche' },
      {
        ...
        render: (row) => {
          return row.date_lancement
            ? new Date(row.date_lancement).toLocaleDateString('fr-FR')
            : '';
        },
      },
      { key: 'niveau_etages', label: "Niveau d'étages" },
      {
        ...
        render: (row) => {
          return row.date_livraison
            ? new Date(row.date_livraison).toLocaleDateString('fr-FR')
            : '';
        },
      },
      {
    ...
    exportConfig: (items) => ({
      data_to_export: items.map((item) => ({
        Tranche: item.nom || '',
        'Date lancement': item.date_lancement
          ? new Date(item.date_lancement).toLocaleDateString('fr-FR')
          : '',
        "Niveau d'étages": item.niveau_etages || '',
        'Date livraison': item.date_livraison
      ...
      columns_export: [
        ...
        { key: 'Date lancement', label: 'Date lancement' },
        { key: "Niveau d'étages", label: "Niveau d'étages" },
        { key: 'Date livraison', label: 'Date livraison' },
      ],
  ...
  immeuble: {
    ...
    exportConfig: (items, nbre_tranches, nbre_blocs) => ({
      ...
      columns_export: [
        ...
        { key: 'Titre foncier', label: 'Titre foncier' },
        { key: 'Nbr Biens', label: 'Nombre Biens' },
      ],
 ...
 const paginatedItems = useMemo(() => {
  ...
  const currentColumns = useMemo(() => {
    ...
    if (typeof columnConfig === 'function') {
      switch (activeTab) {
        ...
        case 'immeuble':
          return columnConfig(user, handleDelete, nbre_tranches, nbre_blocs);
        case 'bien':
          return columnConfig(
            user,
            handleDelete,
    ...
    if (dropZone && file) {
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="text-left">
          ...
          <p class="text-xs text-gray-500">${(file.size / 1024 / 1024).toFixed(
            2
          )} MB</p>
        </div>
      </div>
  ...
  const sendMassEditTitreDataToBackend = async (jsonData, file) => {
    try {
      ...
      dataToSend.append('projet_id', projectId);
        ...
        try {
          ...
          if (rows.length > 0) {
            for (let i = 0; i < rows.length; i++) {
              ...
              // NEW: Validate Type ID - should be numeric
            ...
            if (!typeId && typeId !== 0) {
              ...
              err = 1;
            } else if (typeId && (isNaN(parseFloat(typeId)) || !isFinite(typeId))) {
              msg_error.push({
                id: i + 1,
                msg: `Ligne ${i + 1}: Type  doit être un nombre valide (ex: 1, 2, 3). Valeur actuelle: "${typeId}"`,
              });
              err = 1;
            }

            const typologieId = row['Typologie'];
            if (typologieId && typologieId !== '') {
              if (isNaN(parseFloat(typologieId)) || !isFinite(typologieId)) {
                msg_error.push({
                  id: i + 1,
                  msg: `Ligne ${i + 1}: Typologie doit être un nombre valide ou vide. Valeur actuelle: "${typologieId}"`,
                });
                err = 1;
              }
            }

            // NEW: Validate Vue ID - should be numeric (can be empty)
            const vueId = row['Vue'];
            if (vueId && vueId !== '') {
              if (isNaN(parseFloat(vueId)) || !isFinite(vueId)) {
                msg_error.push({
                  id: i + 1,
                  msg: `Ligne ${i + 1}: Vue  doit être un nombre valide ou vide. Valeur actuelle: "${vueId}"`,
                });
                err = 1;
              }
            }
            ...
            <Modal isVisible={true} onClose={() => setShowDeleteModal(false)}>
              <DeleteData
                ...
                message={`Êtes-vous sûr de vouloir supprimer ${
                  TAB_CONFIG[safeActiveTab]?.name === 'Tranches'
                    ? 'cette'
                    : TAB_CONFIG[safeActiveTab]?.name === 'Immeubles'
                    ? 'cet'
                    : 'ce'
                } ${
                  TAB_CONFIG[safeActiveTab]?.name.endsWith('s')
