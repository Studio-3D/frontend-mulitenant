...
export const GeneralParametersStep = ({ 
  ...
  useEffect(() => {
    if (!isInitialized && formData.parameters.utilisateursAcces && users.length > 0) {
      // Convert all IDs to strings for consistency
      const userIds = formData.parameters.utilisateursAcces.map(id => id.toString());
        ...
        {displayItems.length > 0 && (
          <div className="mt-2">
            <h4 className="text-sm font-medium text-gray-700 mb-2">
              {editMode ? `${label} existants` : `${label} ajoutés`} ({displayItems.length})
            </h4>
            <div className="flex flex-wrap gap-2">
              {displayItems.map((item, index) => (
                <div key={index} className="flex items-center bg-gray-100 rounded-md px-3 py-1">
                  <span className="text-sm">{getItemDisplayValue(item, field)}</span>
                  <button
                    type="button"
          ...
          <div>
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex gap-2">
                <div className="flex-1 flex gap-2">
                  <input
                    ...
                    onChange={(e) => setPartenaireInputs(prev => ({ ...prev, description: e.target.value }))}
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border px-3 py-2"
                  />
                  <input
                    type="number"
                    ...
                    onChange={(e) => setPartenaireInputs(prev => ({ ...prev, remise: e.target.value }))}
              ...
              {(formData.parameters.partenaires || []).length > 0 && (
                <div className="mt-2">
                  <h4 className="text-sm font-medium text-gray-700 mb-2">
                    {editMode ? 'Partenaires existants' : 'Partenaires ajoutés'} ({formData.parameters.partenaires.length})
                  </h4>
                  <div className="flex flex-col gap-2">
                    {formData.parameters.partenaires.map((partenaire, index) => (
                      <div key={index} className="flex items-center justify-between bg-gray-100 rounded-md px-3 py-2">
                        <div>
                          <span className="font-medium">{getItemDisplayValue(partenaire, 'partenaires')}</span>
                          {partenaire.remise && (
                        ...
                        <button
                          ...
                          aria-label={`Retirer ${getItemDisplayValue(partenaire, 'partenaires')}`}
                        >
                          <XIcon size={14} />
                        </button>
                      </div>
            ...
            <div className="space-y-3 mb-6">
              ...
              {selectedUsers.length > 0 && (
                <div className="mt-3">
                  <h4 className="text-sm font-medium text-gray-700 mb-2">
                    {editMode ? 'Utilisateurs existants' : 'Utilisateurs ajoutés'} ({selectedUsers.length})
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {selectedUsers.map(user => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const updatedIds = selectedUserIds.filter(id => id !== user.id.toString());
