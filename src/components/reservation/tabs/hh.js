...
const AddRdvModal = ({ open, reservation_id, onClose, onRdvAdded }) => {
  ...
  const fetchEvents = async (fetchInfo = {}) => {
    ...
    const end = fetchInfo.end || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
  ...
  const handleSlotSelect = async (selectInfo) => {
    ...
    if (selectInfo.start < now) {
      setErrors({ general: ["Vous ne pouvez pas sélectionner une date passée"] });
      return;
    }
    ...
    setSelectedSlot({
      ...
      startTime: selectInfo.start.toLocaleTimeString('fr-FR', {
        hour: '2-digit',
        minute: '2-digit',
      }),
      endTime: selectInfo.end.toLocaleTimeString('fr-FR', {
        hour: '2-digit',
        minute: '2-digit',
      }),
    });
  ...
  const handleDetailSubmit = async () => {
    if (!type) {
      setErrors({ general: ['Veuillez sélectionner un type de rendez-vous'] });
      return;
    }
      ...
      {showDetailModal && selectedSlot && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-500">
                      {selectedSlot.display} • {selectedSlot.startTime} - {selectedSlot.endTime}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-gray-50 px-4 py-3 flex justify-end space-x-2 border-t rounded-b-xl">
              <button
                type="button"
                onClick={handleCloseDetail}
