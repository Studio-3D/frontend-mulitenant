...
filters: (tabsData, blocId, nbre_tranches, tranchesArray) => [
      {
        key: 'nom',
        label: 'Nom',
        type: 'text',
    ...
    exportConfig: (items, nbre_tranches) => ({
      data_to_export: items.map((item) => ({
        Immeuble: item.nom || '',
        Projet_nom: item.nom_projet || '',
        Bloc_nom: item.nom_bloc || '',
  ...
  filters: (tabsData, blocId, nbre_immeubles, nbre_tranches, max_etages, typologies, vues, typeBiens,tranchesArray, immeublesArray) => {
          ...
          ? [
              {
                ...
                options: (Array.isArray(immeublesArray) ? immeublesArray : [immeublesArray]).filter(Boolean).map((b) => ({
                   label: b.nom,
                    value: b.nom,
                  })) || [],
                className:
...
/* import titre foncier*/
  ...
  }, [activeTab, user, handleDelete, nbre_immeubles, nbre_tranches]);
          ...
          <Table
            ...
            emptyMessage={
              <div className="flex flex-col items-center justify-center py-12 text-gray-500">
                <p className="text-sm">Aucune donnée disponible pour ce bloc</p>
              </div>
            }
