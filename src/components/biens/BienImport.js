...
// Validation for Vue (should be numeric ID)
...
// Validation for Type bien (should be numeric ID)
...
const validateEtage = (value) => {
  ...
  // Check for "1er étage" format
  if (normalizedValue === '1er étage' || normalizedValue === '1er etage') return true;
  
  // Check for "Xème étage" format (2ème étage, 3ème étage, etc.)
...
const handleClearFile = () => {
            ...
            if ('Superficie architecte' in row && 'Superficie habitable' in row) {
              ...
              msg_error.push({
               ...
               hasError = true;
                ...
                // NEW: Validate Typologie
              const typologieValue = row['Typologie'];
              if (typologieValue && !validateTypologie(typologieValue)) {
                msg_error.push({
                  id: i,
                  msg: `La ligne ${i + 1} : la colonne "Typologie" doit contenir un ID numérique valide (ex: 1, 2, 3). Valeur actuelle: "${typologieValue}"`,
                });
                hasError = true;
              }
              ...
              if (vueValue && !validateVue(vueValue)) {
                msg_error.push({
                  id: i,
                  msg: `La ligne ${i + 1} : la colonne "Vue" doit contenir un ID numérique valide (ex: 1, 2, 3). Valeur actuelle: "${vueValue}"`,
                });
                hasError = true;
              }

              // NEW: Validate Type bien
              const typeBienValue = row['Type bien'];
              if (typeBienValue && !validateTypeBien(typeBienValue)) {
                msg_error.push({
                  id: i,
                  msg: `La ligne ${i + 1} : la colonne "Type bien" doit contenir un ID numérique valide (ex: 1, 2, 3). Valeur actuelle: "${typeBienValue}"`,
                });
                hasError = true;
              }
              ...
              if (etageValue && !validateEtage(etageValue)) {
                msg_error.push({
                  id: i,
                  msg: `La ligne ${i + 1} : la colonne "Etage" doit contenir une valeur valide (ex: RDC, 1er étage, 2ème étage, etc.). Valeur actuelle: "${etageValue}"`,
                });
                hasError = true;
              }
      }
  ...
  const handleSubmitTypologie = () => {
    if (!validateTypologieInputs()) {
      setTypologiesBackendErrors(
        'Veuillez remplir toutes les typologies avant de enregistrer.'
      );
      return;
    }
  ...
  const handleSubmitVue = () => {
    if (!validateVueInputs()) {
      setVuesBackendErrors(
        'Veuillez remplir toutes les vues avant de enregistrer.'
      );
      return;
    }
                  ...
                  {inputs.map((item, index) => (
                    ...
                    >
                      ...
                      <TextField
                        ...
                        errors={{}}
                        backendErrors={null}
                        onChange={(e) => handleTypeBienChange(e, index)}
                        width="w-full"
                      ...
                      {inputs.length > 1 && (
                        <button
                          type="button"
                          onClick={() => handleDeleteTypeBienInput(index)}
                      ...
                      {index === inputs.length - 1 && (
                        <button
                          type="button"
                          onClick={handleAddTypeBienInput}
              ...
              <p className="text-indigo-700 font-semibold mb-2">
               ...
               {(typologies_get && typologies_get.length > 0) || typologies.length > 0 ? (
                ...
                <table className="min-w-full bg-white rounded shadow text-sm text-left">
                  <thead className="bg-blue-500 text-white">
                    <tr>
                      <th className="px-4 py-2">Typologie</th>
                      <th className="px-4 py-2">
                        Nombre à renseigner dans le fichier
                      </th>
                    </tr>
                ...
                <>
                  <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4">
                    <p className="text-center">
                      Veuillez ajouter les typologies pour ce projet.
                    </p>
                  </div>
                  ...
                  {typologiesInputs.map((item, index) => (
                    ...
                    >
                      ...
                      <TextField
                        ...
                        name="typologie"
                        type="text"
                        value={item.typologie}
                        control={false}
                        errors={{}}
                        backendErrors={null}
                        onChange={(e) => handleTypologieChange(e, index)}
                        width="w-full"
                        placeholder={`Entrez la typologie ${index + 1}`}
                      />
                      {typologiesInputs.length > 1 && (
                        <button
                          type="button"
                          onClick={() => handleDeleteTypologieInput(index)}
                          className="text-red-600 hover:text-red-800 p-1 flex-shrink-0"
                        >
                          <Trash2 size={20} />
                        </button>
                      )}
                      {index === typologiesInputs.length - 1 && (
                        <button
                          type="button"
                          onClick={handleAddTypologieInput}
                          className="text-green-600 hover:text-green-800 p-1 flex-shrink-0"
                        >
                          <PlusSquare size={20} />
                        </button>
                  ...
                  <div className="flex justify-end mt-4 w-full">
                    <Button1
                      type="button"
                      variant="contained"
                      onClick={handleSubmitTypologie}
                      disabled={typologiesLoading || !areAllTypologiesFilled}
                      className={`${
                        typologiesLoading || !areAllTypologiesFilled
                          ? 'bg-gray-400 cursor-not-allowed'
                          : 'bg-blue-600 hover:bg-blue-700'
                      } text-white`}
                    >
          ...
          <tr key={vue.id || index} className="border-b">
            ...
            <td className="px-4 py-2">{vue.id}</td>
                ...
                <>
                  <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4">
                    <p className="text-center">
                      Veuillez ajouter les vues pour ce projet.
                    </p>
                  </div>
                  ...
                  {vuesInputs.map((item, index) => (
                    ...
                    >
                      ...
                      <TextField
                        ...
                        errors={{}}
                        backendErrors={null}
                        onChange={(e) => handleVueChange(e, index)}
                        width="w-full"
                        placeholder={`Entrez la vue ${index + 1}`}
                      />
                      {vuesInputs.length > 1 && (
                        <button
                          type="button"
                          onClick={() => handleDeleteVueInput(index)}
                          className="text-red-600 hover:text-red-800 p-1 flex-shrink-0"
                        >
                          <Trash2 size={20} />
                        </button>
                      )}
                      {index === vuesInputs.length - 1 && (
                        <button
                          type="button"
                          onClick={handleAddVueInput}
                          className="text-green-600 hover:text-green-800 p-1 flex-shrink-0"
                        >
                          <PlusSquare size={20} />
                        </button>
                  ...
                  <div className="flex justify-end mt-4 w-full">
                    <Button1
                      type="button"
                      variant="contained"
                      onClick={handleSubmitVue}
                      disabled={vuesLoading || !areAllVuesFilled}
                      className={`${
                        vuesLoading || !areAllVuesFilled
                          ? 'bg-gray-400 cursor-not-allowed'
                          : 'bg-blue-600 hover:bg-blue-700'
                      } text-white`}
                    >
