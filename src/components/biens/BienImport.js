...
const handleDialogToggle = () => {
      ...
      if (rows.length > 0) {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          if ('Superficie totale' in row && 'Superficie habitable' in row) {
            if (row['Superficie totale'] == 0 && row['Superficie habitable'] == 0) {
              msg_error.push({
                id: i,
                msg: `La ligne ${i + 1} ne dispose d'aucune surface existante, qu'elle soit habitable ou totale.`
              })
              err = 1
            }
          } else if (!('Superficie totale' in row) && 'Superficie habitable' in row) {
          ...
          } else if (!('Superficie habitable' in row) && 'Superficie totale' in row) {
        ...
        setBackendErrors([{ id: 1, msg: 'Le fichier est vide. Veuillez renseigner les colonnes.' }])
        setDisabled_var(true)
      }
    }
    ...
    if (err == 0) {
      ...
      axios
        ...
        .then(() => {
          ...
          toast.success(`Le fichier est en cours d'importation,consulter le Menu Importation des ficiher`)
  ...
  axios.post(`${APIURL.ROOTV1}/store_multiple_type_biens`, formData, {
    headers: {
      Authorization: `Bearer ${localStorage.getItem('accessToken')}`
    }
  })
  ...
  <DialogContent className="bg-blue-50 px-10 py-8">
    <form onSubmit={onSubmit_file} className="flex flex-col items-center w-full max-w-3xl mx-auto gap-6">
      ...
      <div className="w-full">
        ...
        {(Object.keys(selectedProjet?.types_bien).length > 0 || type_biens.length > 0) ? (
          <table className="min-w-full bg-white rounded shadow text-sm text-left">
            <thead className="bg-blue-500 text-white">
              <tr>
                <th className="px-4 py-2">Type de Bien</th>
                <th className="px-4 py-2">Nombre à renseigner dans le fichier</th>
              </tr>
            </thead>
              ...
              {selectedProjet?.types_bien &&
                Object.keys(selectedProjet.types_bien).map(key => (
                  <tr key={key} className="border-b">
                    <td className="px-4 py-2">{selectedProjet.types_bien[key].type}</td>
                    <td className="px-4 py-2">{selectedProjet.types_bien[key].id}</td>
                  </tr>
                ))}
              {type_biens.map(key => (
          ...
          <>
            ...
            {inputs.map((item, index) => (
              <div key={index} className="flex items-center gap-3 mt-4 w-full">
                ...
                />
                {inputs.length > 1 && (
                  <Button onClick={() => handleDeleteInput(index)} title="Supprimer" className="text-red-600">
                    <Trash2 size={20} />
                  </Button>
                )}
                {index === inputs.length - 1 && (
                  <Button onClick={handleAddInput} title="Ajouter +" className="text-green-600">
                    <PlusSquare size={20} />
                  </Button>
                )}
              </div>
            ...
            {backendErrors_tp && <p className="text-red-600">{backendErrors_tp}</p>}
      ...
      <div className="flex gap-4 justify-end w-full mt-6">
        <Button1
          ...
          disabled={disabled_var || (Object.keys(selectedProjet.types_bien).length === 0 && type_biens.length === 0)}
        >
          Enregistrer
        </Button1>
        <Button1
