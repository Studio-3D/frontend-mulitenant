...
export default function BienForm({ id, projetId, blocId, immeubleId }) {
  ...
  const handleChange = (field, value) => {
    ...
    if (field === "tranche_id") {
      // When tranche changes, reset bloc and immeuble selections immediately
      setFormData(prev => ({
        ...prev,
        bloc_id: "",
        immeuble_id: ""
      ...
      // Reset filtered collections and selections
      setSelectedBloc(null);
      setSelectedImmeuble(null);
      setFilteredImmeubles([]);
      
      // If a tranche is selected, update selected tranche and fetch its blocs
      if (value) {
        ...
        // If no tranche selected, clear all related data
        setFilteredBlocs([]);
        setSelectedTranche(null);
      }
    } else if (field === "bloc_id") {
      // When bloc changes, reset immeuble selection
      setFormData(prev => ({
        ...prev,
        immeuble_id: ""
      }));
      ...
      // If a bloc is selected, fetch its immeubles and update the selected bloc
      if (value) {
