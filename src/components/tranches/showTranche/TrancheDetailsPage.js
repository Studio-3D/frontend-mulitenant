...
const getStoredActiveTab = (trancheId) => {
  if (typeof window === 'undefined') return null;
  const stored = localStorage.getItem(`tranche-${trancheId}-activeTab`);
  return stored || null;
};

const setStoredActiveTab = (trancheId, tabName) => {
  if (typeof window === 'undefined') return;
  localStorage.setItem(`tranche-${trancheId}-activeTab`, tabName);
};
export const TrancheDetailsPage = () => {
  ...
  const allTabsData = useMemo(() => {
    ...
    const immeublesData = showImmeubles
      ? trancheData.tranche.immeuble || []
      : [];
    ...
    const immeubles =
      immeublesData.map((i) => ({
        ...
        nom_projet: trancheData?.tranche?.projet?.nom,
        nom_tranche: trancheData?.tranche?.nom,
      })) || [];
